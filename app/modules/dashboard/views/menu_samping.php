    ...
    <ul class="nav">
        ...
        if($this->session->userdata('level')=='0'){
            ...
            foreach ($menusub as $key ) {
                $menusba = array(
                    'nama_smenu' =>$key->nama_smenu,
                    'sicon'      =>$key->sicon,
                    'anak'       =>$key->anak,
                    'parent'     =>$key->parent,
                    'smenu_id'   =>$key->smenu_id,
                    'link'       =>$key->slink);
            ...
            $menusbbx = array();
            $menusbax = array();
            $menu = $this->db->query("SELECT * FROM tbl_menu WHERE status = '1' ORDER BY urutan ASC")->result();
            foreach ($menu as $key) {
                $pmen = array(
                    'nama_menu' =>$key->nama_menu,
                    'link'      =>$key->link,
