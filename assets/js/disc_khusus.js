...
$(function() {
    ...
    table = $('#data-disc_khusus').DataTable({ 
        autoWidth: false,
        columnDefs: [{
            width: '5',
            targets: 0,
        ...
        "pageLength": 10,
        "ajax": {
            "url": $BASE_URL+ "disc_khusus/getData",
            "type": "POST"
        },
...
function tambah_data(){
    ...
    $('.modal-title').text('Tambah Data Diskon Khusus');
}
...
function edit_data(page,link,action,id){ 
    ...
    $.ajax({
        ...
        success: function(data){
            $('[name="nama"]').val(data.nama_diskon);
            $('[name="diskon"]').val(data.diskon);
            var tgl_mulai = new Date(data.tgl_mulai); 
            ...
            $('[name="id"]').val(data.id);
            $('#modal_form').modal('show');
            $('.modal-title').text('Edit Data Diskon Khusus');
